...
func getDnsmasqHostsPathTemp() string {
	return filepath.Join(utils.GetZvrRootPath(), "dnsmasq.hosts")
}
...
func NewDnsmasq(nics, servers map[string]string, records []DnsRecord) *DnsmasqConf {
	...
	dnsRecords := append([]DnsRecord{}, records...)

	return &DnsmasqConf{NicNames: nicNames, DnsServers: ips, DnsRecords: dnsRecords}
}
...
func (d *DnsmasqConf) RestartDnsmasq() error {
	...
	err = templateHandler(d, getDnsmasqHostsPathTemp(), "dnsmasq.hosts", hostsTemplate)
	utils.PanicOnError(err)
	bash = utils.Bash{
		Command: fmt.Sprintf("mv %s %s", getDnsmasqHostsPathTemp(), DNSMASQ_HOSTS_FILE),
		Sudo:    true,
	}
	err = bash.Run()
	utils.PanicOnError(err)
