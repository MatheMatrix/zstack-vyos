...
type SlbHaTestEnv struct {
	...
	// Backend Server 配置
	BS1 BackendServerInfo
	BS2 BackendServerInfo
	BS3 BackendServerInfo
	BS4 BackendServerInfo
}
...
func (env *SlbHaTestEnv) SetupVyosHA() interface{} {
	...
	vyosHaCmd := &SetVyosHaCmd{
		...
		Monitors:     []string{},
		Vips:         []MacVipPair{vip4, vip6},
	}

	return SetVyosHa(vyosHaCmd)
}

// SetupLoadBalancer 设置负载均衡配置
func (env *SlbHaTestEnv) SetupLoadBalancer() error {
	InitLb()
	os.Remove(IPVS_HEALTH_CHECK_CONFIG_FILE)
	InitIpvs()
	...
	// 配置 Backend Servers
	env.BS1 = BackendServerInfo{
		Ip:     "192.168.3.10",
		Weight: 100,
	}

	env.BS2 = BackendServerInfo{
		Ip:     "192.168.3.11",
		Weight: 100,
	}

	// 配置 Server Group
	env.SG = ServerGroupInfo{
		Name:            "default-server-group",
		ServerGroupUuid: "8e52bcc526074521894162aa8db73c24",
		BackendServers:  []BackendServerInfo{env.BS1, env.BS2},
		IsDefault:       false,
	}

	env.Lb.ServerGroups = []ServerGroupInfo{env.SG}
	...
	env.BS3 = BackendServerInfo{
		Ip:     "192.168.3.12",
		Weight: 100,
	}

	env.SG1 = ServerGroupInfo{
		Name:            "server-group-1",
		ServerGroupUuid: "8e52bcc526074521894162aa8db73c25",
		BackendServers:  []BackendServerInfo{env.BS3},
		IsDefault:       false,
	}

	env.Lb1.ServerGroups = []ServerGroupInfo{env.SG1}
